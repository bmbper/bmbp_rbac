...
export const PageAction = {
  ...
  findTreeData: (v: String | null) => {
    ...
    axios
      .post(PageUrl.findTreeUrl, { menuName: v })
      .then((resp: any) => {
  ...
  addChildNode: (node: any) => {
    ...
    if (node && node.menuCode) {
      ...
      if (
        ...
        PageState.selectTreeNodeData.menuCode
      ) {
        menuParentCode = PageState.selectTreeNodeData.menuCode;
      }
    }
  ...
  save(menuData: any, callback: () => void) {
    axios.post(PageUrl.saveUrl, menuData).then((resp: any) => {
      if (resp.code == 0) {
  ...
  updateParent(menuData: any, callback: () => void) {
    axios.post(PageUrl.updateParentUrl, menuData).then((resp: any) => {
      if (resp.code == 0) {
        arco.Message.success(resp.msg);
        callback();
      } else {

...
export const PageAction = {
  ...
  findTreeData: (v: String | null) => {
    ...
    axios
      .post(PageUrl.findTreeUrl, { userName: v })
      .then((resp: any) => {
  ...
  addChildNode: (node: any) => {
    ...
    if (node && node.userCode) {
      ...
      if (
        ...
        PageState.selectTreeNodeData.userCode
      ) {
        userParentCode = PageState.selectTreeNodeData.userCode;
      }
    }
  ...
  save(userData: any, callback: () => void) {
    axios.post(PageUrl.saveUrl, userData).then((resp: any) => {
      if (resp.code == 0) {
  ...
  updateParent(userData: any, callback: () => void) {
    axios.post(PageUrl.updateParentUrl, userData).then((resp: any) => {
      if (resp.code == 0) {
        arco.Message.success(resp.msg);
        callback();
      } else {

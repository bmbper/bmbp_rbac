...
export const PageAction = {
  ...
  findTreeData: (v: String | null) => {
    ...
    axios
      .post(PageUrl.findTreeUrl, { roleName: v })
      .then((resp: any) => {
  ...
  addChildNode: (node: any) => {
    ...
    if (node && node.roleCode) {
      ...
      if (
        ...
        PageState.selectTreeNodeData.roleCode
      ) {
        roleParentCode = PageState.selectTreeNodeData.roleCode;
      }
    }
  ...
  save(roleData: any, callback: () => void) {
    axios.post(PageUrl.saveUrl, roleData).then((resp: any) => {
      if (resp.code == 0) {
  ...
  updateParent(roleData: any, callback: () => void) {
    axios.post(PageUrl.updateParentUrl, roleData).then((resp: any) => {
      if (resp.code == 0) {
        arco.Message.success(resp.msg);
        callback();
      } else {
